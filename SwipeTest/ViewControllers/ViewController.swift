...
KolodaViewDataSource,KolodaViewDelegate, ViewControllerDelegate {
...
//    @IBOutlet var titleImageView: UIImageView!
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.titleView = UIImageView(image:UIImage(named:"photton9.png"))
        ...
        PHPhotoLibrary.requestAuthorization { (status) -> Void in
            ...
            case .Denied: //許可されていない
                dispatch_async(dispatch_get_main_queue(), {
                    ...
                    let alert = UIAlertController(title: "写真へのアクセス許可", message: "許可なしで使用することは出来ません", preferredStyle: .Alert)
                    
                    let okAction = UIAlertAction(title: "設定", style: .Default) {
                        action in NSLog("OK！")
        ...
        let longPressGesture = UILongPressGestureRecognizer(target: self, action: #selector(ViewController.doGesture))
        self.deleteButton.addGestureRecognizer(longPressGesture)
...
//        imageView.layer.borderColor = UIColor(red: 27/255, green: 20/255, blue: 100/255, alpha: 1).CGColor
    ...
    func kolodaDidRunOutOfCards(koloda: KolodaView) {
        ...
        for _ in 0 ..< self.photoAssets.count {
    ...
    func kolodaDraggedCard(koloda: KolodaView, finishPercent: CGFloat, direction: SwipeResultDirection) {
        ...
        saveview.frame = CGRectMake(144, 20, 150, 100)
        dumpview.frame = CGRectMake(-3, 20, 150, 100)
    ...
    @IBAction func tapYES() {
        ...
        let dumpview = UIImageView()
        dumpview.frame = CGRectMake(-3, 20
            , 150, 100)
        let img = UIImage(named:"dump")
        dumpview.image = img
        dumpview.transform = CGAffineTransformMakeRotation(-45 * CGFloat(M_PI/180))
        kolodaView.viewForCardAtIndex(kolodaView.currentCardNumber)?.addSubview(dumpview)
    ...
    @IBAction func tapNO() {
        ...
        saveview.image = img
        saveview.transform = CGAffineTransformMakeRotation(45 * CGFloat(M_PI/180))
        kolodaView.viewForCardAtIndex(kolodaView.currentCardNumber)?.addSubview(saveview)
        ...
        PHPhotoLibrary.sharedPhotoLibrary().performChanges({ () -> Void in
            PHAssetChangeRequest.deleteAssets(self.stackedAssets)
                ...
                if error != nil {
                    ...
                    self.stackedAssets.removeAll()
                    
                    dispatch_async(dispatch_get_main_queue(), {
                        self.kolodaView?.reloadData()
                        self.stackedNumberLabel.text = String(self.stackedAssets.count)
    ...
    func doGesture() {
        let actionSheet:UIAlertController = UIAlertController(title:"Trash box",
                                                              message: "",
        ...
        //Default 複数指定可
        let defaultAction:UIAlertAction = UIAlertAction(title: "確認する",
                                                        style: UIAlertActionStyle.Default,
                                                        handler:{
                                                            (action:UIAlertAction!) -> Void in
                                                            
                                                            self.performSegueWithIdentifier("toStackedImageVC", sender: nil)
    ...
    func checkStackedAssets(stackedAssets: [PHAsset]) {
        self.stackedAssets = stackedAssets
        stackedNumberLabel.text = String(stackedAssets.count)
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        let stackedImageView = segue.destinationViewController as! stackedImageViewController
        stackedImageView.stackedAssets = self.stackedAssets

        stackedImageView.delegate = self
    }
